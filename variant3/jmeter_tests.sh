...
# 🔥 Script de tests JMeter pour Variante D
# Auteur: Halmaoui Abdellah
# Date: Novembre 2025

echo "🔥 TESTS JMETER - VARIANTE D (Spring Data REST)"
echo "================================================="
echo ""
...
if ! curl -s "$BASE_URL/items" > /dev/null 2>&1; then
    ...
    echo "Veuillez démarrer le serveur avec: cd variant-d && mvn spring-boot:run"
    exit 1
fi
